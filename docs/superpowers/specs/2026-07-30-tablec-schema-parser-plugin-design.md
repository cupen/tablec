...
| parser 动态库加载（cdylib） | 安全 / ABI 复杂度 | 本 spec 不做；明确"未来 spec"路径 |
...
13. 移除 `Schema::from_parts`(所有构造点应改为结构体字面量)；下个 spec 收
14. 各阶段提交前 bd close 对应 issue; git push 走 feat/xxxx 分支
```
