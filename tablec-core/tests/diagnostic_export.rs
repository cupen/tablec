...
fn read_excel_propagates_diagnostics_on_bad_value() {
    ...
    let n = errs.iter().filter(|d| d.code == tablec_core::core::diagnostic::DiagnosticCode::ValueOutOfRange).count();
...
fn read_excel_propagates_struct_field_mismatch() {
    ...
    let errs = tablec_core::core::table::table::read_excel(path.to_str().unwrap())
        .err()
        .expect("expected Err");
    assert!(errs.iter().any(|d| d.code == tablec_core::core::diagnostic::DiagnosticCode::StructFieldMismatch
