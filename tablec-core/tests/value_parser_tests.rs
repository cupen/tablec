...
fn test_parse_int_positive() {
    assert_eq!(
        parse_value("42", &FieldType::Int32, loc()).unwrap(),
        Value::Int32(42)
    );
}
...
fn test_parse_int_negative() {
    assert_eq!(
        parse_value("-7", &FieldType::Int32, loc()).unwrap(),
        Value::Int32(-7)
    );
}
...
fn test_parse_int_zero() {
    assert_eq!(
        parse_value("0", &FieldType::Int32, loc()).unwrap(),
        Value::Int32(0)
    );
}
...
fn test_parse_int_large() {
    assert_eq!(
        parse_value("9223372036854775807", &FieldType::Int64, loc()).unwrap(),
...
fn test_parse_uint() {
    assert_eq!(
        parse_value("42", &FieldType::Uint32, loc()).unwrap(),
        Value::Uint32(42)
    );
}
...
fn test_parse_uint_zero() {
    assert_eq!(
        parse_value("0", &FieldType::Uint32, loc()).unwrap(),
        Value::Uint32(0)
    );
}
...
fn test_parse_string_plain() {
    assert_eq!(
        parse_value("hello", &FieldType::String, loc()).unwrap(),
...
fn test_parse_string_quoted_double() {
    assert_eq!(
        parse_value("\"world\"", &FieldType::String, loc()).unwrap(),
...
fn test_parse_string_empty() {
    assert_eq!(
        parse_value("", &FieldType::String, loc()).unwrap(),
...
fn test_parse_bool_true() {
    assert_eq!(
        parse_value("true", &FieldType::Bool, loc()).unwrap(),
        Value::Bool(true)
    );
}
...
fn test_parse_bool_false() {
    assert_eq!(
        parse_value("false", &FieldType::Bool, loc()).unwrap(),
        Value::Bool(false)
    );
}
...
fn test_parse_bool_numeric() {
    ...
    assert_eq!(
        parse_value("1", &FieldType::Bool, loc()).unwrap(),
        Value::Bool(true)
    );
    assert_eq!(
        parse_value("0", &FieldType::Bool, loc()).unwrap(),
        Value::Bool(false)
    );
}
...
fn test_parse_map_int_to_string() {
    ...
    match v {
        Value::Map(m) => {
            assert_eq!(
                m.get(&Value::Int32(1)).unwrap(),
                &Value::String("one".to_string())
            );
            assert_eq!(
                m.get(&Value::Int32(2)).unwrap(),
...
fn test_parse_map_string_to_int() {
    ...
    match v {
        Value::Map(m) => {
            assert_eq!(
                m.get(&Value::String("k1".to_string())).unwrap(),
                &Value::Int32(1)
            );
            assert_eq!(
                m.get(&Value::String("k2".to_string())).unwrap(),
                &Value::Int32(2)
            );
        }
        _ => panic!("Expected Map"),
...
fn test_parse_struct_wrong_field_count() {
    ...
    assert_eq!(
        err.code,
        tablec_core::core::diagnostic::DiagnosticCode::StructFieldCountMismatch
    );
}
