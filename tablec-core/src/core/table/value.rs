            ...
            Value::Map(m) => {
                ...
                for (k, v) in m {
                    let key_str = match k {
                        ...
                        _ => {
                            return Err(serde::ser::Error::custom("Map keys must be simple types"));
                        }
                    };
...
impl PartialOrd for Value {
    fn partial_cmp(&self, other: &Self) -> Option<Ordering> {
        if let (Some(a), Some(b)) = (self.to_numeric(), other.to_numeric()) {
            ...
            if let (Some(af), Some(bf)) = (numeric_to_f64(a), numeric_to_f64(b)) {
...
fn numeric_to_f64(n: Numeric) -> Option<f64> {
    Some(match n {
        Numeric::I8(v) => v as f64,
        Numeric::I16(v) => v as f64,
        Numeric::I32(v) => v as f64,
        Numeric::I64(v) => v as f64,
        Numeric::U8(v) => v as f64,
        Numeric::U16(v) => v as f64,
...
mod tests {
    ...
    fn hash_includes_discriminant() {
        ...
        assert_ne!(
            h1.finish(),
            h2.finish(),
            "Int32(0) and Uint32(0) must hash differently"
        );
    }
