...
use crate::core::table::value::Value::{
    Float32, Float64, Int8, Int16, Int32, Int64, String as StrVariant, Uint8, Uint16, Uint32,
    Uint64,
};
...
pub fn parse_value(s: &str, ty: &FieldType, loc: SourceLocation) -> Result<Value, Diagnostic> {
    fn parse_basic(s: &str, ty: &FieldType, loc: &SourceLocation) -> Result<Value, Diagnostic> {
        let trimmed = s.trim();
        match ty {
            FieldType::Int8 => trimmed
                .parse::<i8>()
                .map(Int8)
                .map_err(|_| out_of_range(trimmed, "int8", -128, 127, loc)),
            FieldType::Int16 => trimmed.parse::<i16>().map(Int16).map_err(|_| {
                out_of_range(trimmed, "int16", i16::MIN as i128, i16::MAX as i128, loc)
            }),
            FieldType::Int32 => trimmed.parse::<i32>().map(Int32).map_err(|_| {
                out_of_range(trimmed, "int32", i32::MIN as i128, i32::MAX as i128, loc)
            }),
            FieldType::Int64 => trimmed.parse::<i64>().map(Int64).map_err(|_| {
                out_of_range(trimmed, "int64", i64::MIN as i128, i64::MAX as i128, loc)
            }),
            FieldType::Int => trimmed
                .parse::<i32>()
                .map(Int32)
                .map_err(|_| out_of_range(trimmed, "int", i32::MIN as i128, i32::MAX as i128, loc)),
            FieldType::Uint8 => trimmed
                .parse::<u8>()
                .map(Uint8)
                .map_err(|_| out_of_range(trimmed, "uint8", 0, u8::MAX as i128, loc)),
            FieldType::Uint16 => trimmed
                .parse::<u16>()
                .map(Uint16)
                .map_err(|_| out_of_range(trimmed, "uint16", 0, u16::MAX as i128, loc)),
            FieldType::Uint32 => trimmed
                .parse::<u32>()
                .map(Uint32)
                .map_err(|_| out_of_range(trimmed, "uint32", 0, u32::MAX as i128, loc)),
            FieldType::Uint64 => trimmed
                .parse::<u64>()
                .map(Uint64)
                .map_err(|_| out_of_range(trimmed, "uint64", 0, u64::MAX as i128, loc)),
            FieldType::Uint => trimmed
                .parse::<u32>()
                .map(Uint32)
                .map_err(|_| out_of_range(trimmed, "uint", 0, u32::MAX as i128, loc)),
            FieldType::Float32 => trimmed
                .parse::<f32>()
                .map(Float32)
                .map_err(|_| parse_fail(trimmed, "float32", loc)),
            FieldType::Float64 => trimmed
                .parse::<f64>()
                .map(Float64)
                .map_err(|_| parse_fail(trimmed, "float64", loc)),
            FieldType::Float => trimmed
                .parse::<f32>()
                .map(Float32)
                .map_err(|_| parse_fail(trimmed, "float", loc)),
            ...
            _ => Err(Diagnostic::new(
                ...
                format!("Unsupported basic type for: {:?}", ty),
                loc.clone(),
            )),
        }
    }
    ...
    match ty {
        ...
        FieldType::Date | FieldType::DateTime | FieldType::Timestamp32 | FieldType::Timestamp64 => {
            parse_basic(s, &FieldType::String, &loc)
        }
        FieldType::Int8
        | FieldType::Int16
        | FieldType::Int32
        ...
        | FieldType::Float64
        | FieldType::Float
        | FieldType::Bool => parse_basic(s, ty, &loc),
...
fn parse_array(s: &str, inner: &FieldType, loc: SourceLocation) -> Result<Value, Diagnostic> {
    ...
    if !t.starts_with('[') || !t.ends_with(']') {
        return Err(Diagnostic::new(
            DiagnosticCode::ValueParseError,
            "Invalid array format (need [a, b, …])".to_string(),
            loc,
        ));
    }
    ...
    for (i, c) in inner_str.chars().enumerate() {
        match c {
            ...
            ']' | '}' | '>' => level -= 1,
            ',' if level == 0 => {
                values.push(parse_value(inner_str[start..i].trim(), inner, loc.clone())?);
                start = i + 1;
            }
            _ => {}
        }
...
fn parse_struct(
    ...
    if !t.starts_with('{') || !t.ends_with('}') {
        return Err(Diagnostic::new(
            DiagnosticCode::ValueParseError,
            "Invalid struct format (need {a: x, b: y})".to_string(),
            loc,
        ));
    }
    let inner_str = &t[1..t.len() - 1];
    ...
    for (i, c) in inner_str.chars().enumerate() {
        match c {
            ...
            ']' | '}' | '>' => level -= 1,
            ',' if level == 0 => {
                fields_str.push(&inner_str[start..i]);
                start = i + 1;
            }
            _ => {}
        }
    ...
    if start < inner_str.len() {
        fields_str.push(&inner_str[start..]);
    }
    ...
    for chunk in fields_str {
        ...
        if parts.len() != 2 {
            return Err(Diagnostic::new(
                ...
                format!("expected 'name: value' but got '{}'", chunk),
                loc.clone(),
            ));
        }
    ...
    for f in fields {
        if !m.contains_key(&f.name) {
            return Err(Diagnostic::new(
                ...
                format!("struct missing field '{}'", f.name),
                loc.clone(),
            ));
        }
    }
...
mod tests {
    ...
    fn parse_int_each_width_ok() {
        assert_eq!(
            parse_value("42", &FieldType::Int8, loc()).unwrap(),
            Value::Int8(42)
        );
        assert_eq!(
            parse_value("42", &FieldType::Int16, loc()).unwrap(),
            Value::Int16(42)
        );
        assert_eq!(
            parse_value("42", &FieldType::Int64, loc()).unwrap(),
            Value::Int64(42)
        );
        assert_eq!(
            parse_value("42", &FieldType::Uint8, loc()).unwrap(),
            Value::Uint8(42)
        );
    }
