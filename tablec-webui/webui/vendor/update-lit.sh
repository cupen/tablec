...
#     sourceMappingURL line, and any `export * from "..."` redirector —
#     those are useless after we inlined the target.
#     esbuild sometimes appends `export{a as X,...}` to the same line as
#     the last code statement (separated by `;`); strip that suffix too
#     so step 3b can find the export on its own line for parsing.
...
# 3b. Build the alias shim by composing two maps:
#       (a) chunk export:  `export{a as X}` — a is the chunk's internal
#           var name, X is the public name it exports under.
#       (b) bundle import: `import{X as Y}` — X is the chunk's external
#           name, Y is the bundle's local binding for it.
#     After inlining both files share scope, so Y (bundle-local) must be
#     bound to a (chunk-internal). Emit `var Y=a;` for each pair.
declare -A EXTERNAL_TO_INTERNAL
for chunk in "${CHUNK_BODIES[@]}"; do
  export_line=$(grep -oE 'export\{[^}]+\}' "$chunk" | head -1 || true)
  [[ -z "$export_line" ]] && continue
  while IFS= read -r pair; do
    [[ -z "$pair" ]] && continue
    internal=$(echo "$pair" | sed -E 's/^[[:space:]]*([A-Za-z_$][A-Za-z0-9_$]*) as ([A-Za-z_$][A-Za-z0-9_$]*).*/\1/')
    external=$(echo "$pair" | sed -E 's/^[[:space:]]*([A-Za-z_$][A-Za-z0-9_$]*) as ([A-Za-z_$][A-Za-z0-9_$]*).*/\2/')
    EXTERNAL_TO_INTERNAL["$external"]="$internal"
  done < <(echo "$export_line" | sed -E 's/^export\{//; s/\}$//' | tr ',' '\n')
done

: > "$TMP/alias_pairs.txt"
grep -oE 'import\{[^}]*\}' "$TMP/bundle.mjs" \
  | grep -oE '[A-Za-z_$][A-Za-z0-9_$]* as [A-Za-z_$][A-Za-z0-9_$]*' \
  | sed -E 's/^([A-Za-z_$][A-Za-z0-9_$]*) as ([A-Za-z_$][A-Za-z0-9_$]*)/\1 \2/' \
  | while read -r external_name bundle_local; do
      internal="${EXTERNAL_TO_INTERNAL[$external_name]:-}"
      if [[ -z "$internal" ]]; then
        echo "ERROR: bundle imports '${external_name}' (as '${bundle_local}') but no chunk exports it" >&2
        exit 1
      fi
      echo "${bundle_local}=${internal}" >> "$TMP/alias_pairs.txt"
    done

ALIASES=$(tr '\n' ',' < "$TMP/alias_pairs.txt" | sed 's/,$//')
if [[ -n "$ALIASES" ]]; then
...
# 3d. Now that the alias shim has been emitted, strip the chunk's
#     `export{...}` lines from the assembled file. The bundle's own final
#     `export{...}` (the public API surface) must stay — that's how
#     app.js's `import{...}` from /static/vendor/lit.js gets resolved.
for chunk in "${CHUNK_BODIES[@]}"; do
  export_line=$(grep -oE 'export\{[^}]+\}' "$chunk" | head -1 || true)
  [[ -z "$export_line" ]] && continue
  # Escape for grep -F: replace `\` and `|` with their literal backslash forms.
  escaped=$(printf '%s' "$export_line" | sed -e 's/\\/\\\\/g' -e 's/|/\\|/g')
  # Delete only that exact line.
  grep -F -x -v "$export_line" "$ASSEMBLED" > "$ASSEMBLED.tmp" \
    && mv "$ASSEMBLED.tmp" "$ASSEMBLED"
done
# Also strip any leftover `export * from "..."` lines (defense in depth —
# 3a already removed these, but if a chunk has multiple only the first
# `^export * from` matches).
sed -i -E '/^export \* from "[^"]+";?$/d' "$ASSEMBLED"
